...
# Lista de instancias Piped más estables (Mayo 2026)
PIPED_NODES = [
...
def search(q: str):
            ...
            with urllib.request.urlopen(req, timeout=10, context=ctx) as r:
                ...
                for item in data.get('items', []):
                    if item.get('type') in ['stream', 'video']:
                        ...
                        video_id = url_part.split('v=')[-1].split('&')[0] if 'v=' in url_part else url_part.strip('/')
    ...
    # Fallback fuerte
    print("⚠️ Usando fallback hardcoded")
    fallback = [
