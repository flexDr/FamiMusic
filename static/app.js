...
const poolDembow = ["Dembow Dominicano 2026 hits", "El Alfa El Jefe official audio", "Rochy RD Dembow clasicos", "Tokischa Dembow Mix", "Angel Dior Chimbala Puro Dembow"];
const poolTrap = ["Trap Latino Mix 2026", "Anuel AA Bryant Myers Trap Clasicos", "Eladio Carrion Solito Trap", "Myke Towers Young Miko Trap", "Luar La L Hades66 Trap"];
...
// --- REPRODUCCIÓN PREMIUM ANTI-FRISADO ---
async function playIndex(index, preserveTime = false, isCrossfade = false) {
    ...
    document.getElementById('trackName').innerText = isCrossfade ? "🤖 Mezclando..." : "Cargando...";
    ...
    try {
        ...
        if (song.offlineUrl && !isVideoMode) {
            ...
            if (!data.url) throw new Error("Bloqueo de red completo");

            mediaUrl = data.url;
            // El backend ahora sí nos dará los segundos reales de la API Europea
        ...
        // RECONEXIÓN BRUTA DE SENSORES (Sincroniza barra y reloj de golpe)
        audio.ontimeupdate = () => {
            // Si la duracionReal falló, leemos dinámicamente el archivo cargado
            ...
            // DISPARADOR TRANSICIÓN INTELIGENTE IA (Faltan 8 segundos)
        ...
        try { if (preserveTime && previousTime > 0) audio.currentTime = previousTime; } catch(err) {}

        if (isCrossfade) {
            let volOut = 1.0;
            let fadeOutInt = setInterval(() => {
                volOut -= 0.1;
                if (volOut <= 0.1) { oldAudio.pause(); oldAudio.volume = 1; clearInterval(fadeOutInt); }
            ...
            let fadeInInt = setInterval(() => {
                volIn += 0.1;
                if (volIn >= 0.9) { audio.volume = 1; clearInterval(fadeInInt); }
        ...
        // Configuración inicial de barra
        if (duracionReal > 0) {
        ...
        fallosSeguidos = 0; // Reseteamos fallos si conecta con éxito
        ...
        if (fallosSeguidos >= 3) {
            document.getElementById('trackName').innerText = "❌ Bloqueo persistente. Reintente.";
            actualizarBotonesPlay(svgPlay);
        } else {
            document.getElementById('trackName').innerText = "❌ Reenrutando canal... Saltando";
            if(isVideoMode) {
                isVideoMode = false;
