...
// Desbloqueo forzado para Safari
...
function crearHTMLTarjeta(s, actionStr) {
    ...
    return `<div class="card" onclick="${actionStr}"><img src="${s.thumb}" onerror="this.src='https://ui-avatars.com/api/?name=Mix&background=random'"><div class="card-title">${title}</div><div class="card-subtitle">${artist}</div></div>`;
}
...
// --- EL NUEVO SISTEMA SOUNDCLOUD (SIN ESPERAS) ---
function playIndex(index, preserveTime = false, isCrossfade = false) {
    ...
    let artistStr = song.title.includes("-") ? song.title.split("-")[0].trim() : "SoundCloud Audio";
    ...
    // CONECTAMOS DIRECTO A SOUNDCLOUD
